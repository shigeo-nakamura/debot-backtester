        ...
        for line in file:
            ...
            if len(parts) == 10:
                try:
                    ...
                    rsi = float(parts[1])
                    rsi_short = float(parts[2])
                    rsi_long = float(parts[3])
                    is_expanding = float(parts[4])
                    trend_type = float(parts[5])
                    is_breakout = float(parts[6])
                    is_crossover = float(parts[7])
                    open_signal_trendfollow = float(parts[8])
                    open_signal_meanreversion = float(parts[9])
